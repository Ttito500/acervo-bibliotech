...
public interface TurmaRepository extends JpaRepository<Turma, Integer> {
    ...
    @Query("SELECT new com.bibliotech.bibliotech.dtos.response.TurmaLeiturasDTO(" +
            ...
            "(SELECT a.nome FROM Aluno a LEFT JOIN Emprestimo e2 ON a.id = e2.aluno.id WHERE a.turma.id = t.id AND e2.dataEmprestimo BETWEEN :dataInicio AND :dataFim AND e2.situacao = 'entregue' GROUP BY a.nome ORDER BY COUNT(e2) DESC LIMIT 1), " +
            "(SELECT COUNT(e2) FROM Aluno a LEFT JOIN Emprestimo e2 ON a.id = e2.aluno.id WHERE a.turma.id = t.id AND e2.dataEmprestimo BETWEEN :dataInicio AND :dataFim AND e2.situacao = 'entregue' GROUP BY a.nome ORDER BY COUNT(e2) DESC LIMIT 1)) " +
            "FROM Turma t " +
